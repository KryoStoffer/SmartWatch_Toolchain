...
static uint32_t DCD_HandleRxStatusQueueLevel_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  {
  case STS_GOUT_NAK:
    //sprintf(usb_debug1,"STS_GOUT_NAK");
    break;
  case STS_DATA_UPDT:
    //sprintf(usb_debug2,"STS_DATA_UPDT");
    if (status.b.bcnt)
    {
    ...
    break;
  case STS_XFER_COMP:
    //sprintf(usb_debug3,"STS_XFER_COMP");
    break;
  case STS_SETUP_COMP:
    //sprintf(usb_debug4,"STS_SETUP_COMP");
    break;
  case STS_SETUP_UPDT:
    sprintf(usb_debug5,"STS_SETUP_UPDT %lu %u",count++,status.b.epnum);
    /* Copy the setup packet received in FIFO into the setup buffer in RAM */
    USB_OTG_ReadPacket(pdev , pdev->dev.setup_packet, status.b.bcnt);
    ep->xfer_count += status.b.bcnt;
    break;
  default:
    break;
